...
当提交数据时,使用_parent指定父子关系
#+BEGIN_SRC es
  PUT /index/type1?parent=1
  {
    ...
  }
#+END_SRC
...
#+END_SRC
其中"group_by_tags"是任意取的一个名字,且若聚合的field是分词的field(text),则fielddata属性要设置为true.
若聚合的field是不分词的field(keyword),则fielddata可以不设置 =fielddate=true=.
#+BEGIN_SRC es
...
此外,在aggs中也可以使用 ="filter"= 参数来对聚合进行过滤,类似SQL中的 =having= 语句
#+BEGIN_SRC es
  GET /index/type/_search
  {
    "size":0,
...
#+END_SRC
** 近似聚合算法
*** cardinality去重算法(count(distinct)算法,性能高但有5%的错误率)
#+BEGIN_SRC es
  GET /index/type/_search
  {
    "aggs":{
      "group_by_sold_date":{
        ...
        "aggs":{
          "distinct_brand_count":{
            "cardinality":{
              "field":"brand",
              "precision_threshold":100 // 当brand的总个数在100以内,保证100%准确,每个占用8个字节
            }
          }
        }
      }
...
#+BEGIN_SRC es
  GET /index/type/_search
  {
    ...
    "aggs":{
      "latency_percentiles":{
        "field":"latency",
        "percents":[50,           // 50%记录的latency在多少时间以内
                    95,           // 95记录的latency在多少时间以内
                    99]           // 99%记录的latency在多少时间以内
      }
    }
  }
#+END_SRC
*** percentile_ranks算法(TDigest算法)
#+BEGIN_SRC es
  GET /index/type/_search
  {
    "size":0,
    "aggs":{
      "group_by_province":{
        "terms":{"filed":"province"}
      },
      "aggs":{
        "latency_percentile_ranks":{
          "percentile_ranks":{
            "filed":"latency",
            "values":[200,        // 延时在200以内的百分比
                     1000],       // 延时在1000以内的百分比
            "compression":200     // 设置用多少个节点来计算,节点越多越精准,默认为100,每个节点32个字节
          }
        }
      }
    }
