...
A hook called whenever a window is placed by rule. Arguments are window group
and frame
[Hook] *start-hook*
...
[Hook] *key-press-hook*
A hook called whenever a key under *top-map* is pressed. It is called with 3 argu-ment: the key, the (possibly incomplete) key sequence it is a part of, and command
...
A hook called inside stumpwm’s inner loop, before the default event processing takes
place. This hook is run inside (with-event-queue ...).
