...
** logger命令: 模拟各类消息:
logger命令可以模拟各类消息,从而可以对syslog.conf文件的配置进行测试
#+BEGIN_SRC shell
  logger -h
#+END_SRC
...
#+BEGIN_SRC org
  ...
  Options:
   -a, --all                加载所有 /etc/fstab 中标识为交换分区的分区
   -d, --discard[=<policy>] enable swap discards, if supported by device
   -e, --ifexists           跳过不存在的设备,常与-a连用
   -f, --fixpgsz            reinitialize the swap space if necessary
   -o, --options <list>     comma-separated list of swap options
   -p, --priority <prio>    定义优先级,介于0到32767之间
   -s, --summary            显示交换分区使用信息 (DEPRECATED)
...
| 选项         | 说明                                 |
|--------------+--------------------------------------|
| -c           | 采用C shell命令格式返回设置命令      |
| -u           | 采用Bourne shell命令格式返回设置命令 |
| -s [row col] | 设置屏幕大小                               |
