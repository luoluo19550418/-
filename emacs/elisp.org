...
** 创建子进程
   ...
   参数buffer-orname指定了程序的输出显示在哪个buffer中,nil表示丢弃输出. unless a custom filter function handles it. 对于同步进程,可以指定输出到文件中而不是buffer
...
** process的属性
   ...
   * (set-process-filter process filter)
	 ...
	 filter functiion需要接收2个参数:关联的process和输出的字符串. 
