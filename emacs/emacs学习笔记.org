...
** VC
   1. VC命令汇总
	  ...
	  * C-x v g  / M-x vc-annotate                  ：为文件的每一行都显示一份说明，说明了最后一次更新的时间和人。 可以按l来查看相关的log，按d来查看相关的diff
...
** occur mode
   ...
   * occur-mode的子命令
	 ...
	 * o / C-o 打开当前位置,光标跟随/不跟随过去

	 * e 进入occur-edit-mode进行编辑

	 * C-c C-c 退出occur-edit-mode
...
** 启用goto-address-mode允许你在buffer中高亮并点击URL
...
* NOTE
  ...
  22. 如何关闭emacs中对按键的提示呢? 比如使用M-x helm-occur. 会提示you can run command `helm-occur' with XXXX
      #+BEGIN_SRC emacs-lisp
