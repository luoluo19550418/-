...
** Org-Plot              :org表格绘图
   通过使用'Gnuplot'[[http://www.gnuplot.info]] 和'gnuplot-mode'[[http://cars9.uchicago.edu/~ravel/software/gnuplot-mode.html]] ,Org-Plot可用根据表格中的数据生成2D和3D图表. 这里有一个例子: 首先确保你安装了Gnuplot和Gnuplot mode,然后对下面的表格调用'org-plot/gnuplot'
   #+begin_src org
...
作会议纪要时,可能会需要记录会议开始后的xx时间某某发了一个什么言. Org提供了一个相对计时器的功能可以帮助记录xx时间.
...
Agenda内建和自定义的命令产生的结果是固定的。 但Agenda提供的filter和limit功能提供了两种动态限定agenda entry的方法.
Filter只作用于item的显示上，而limit则在agenda entry创建之前就开始生效了。
Filter常应用于交互式使用时,而limit则常在定义ageneda命令时作为局部变量来用.
...
** 支持的语言
*** SQL
要想执行SQL的代码块,必须先安装好数据库管理系统. 目前Org mode支持以下几种实现:dbi,monetdb,msosql,mysql,postgresql.
**** Header Arguments
~:colnames~ 参数默认为"yes"

SQL支持以下几种特殊的header arguments

+ :engine :: 指定数据库管理系统的实现. 目前Org mode支持以下几种实现:dbi,monetdb,msosql,mysql,postgresql.
+ :cmdline :: 传递给RDBMS可执行文件的额外命令行参数
+ :dbhost :: 数据库服务器主机名
+ :dbuser :: 登录数据库的用户名
+ :dbpassword :: 登录数据库的密码
+ :database :: 数据库名称
...
目前SQL代码块不支持session
*** C / C++ /D
若代码块中没有 =main= 函数,则会将整个代码块包裹在一个 =main= 函数中再执行.
**** Header Arguments

C/C++/D支持以下几种特殊的header arguments

+ :cmdline :: 传递给编译器的额外参数
+ :flags :: 传递给编译器的flags
+ :main :: 设置为"no",则会阻止将整个代码块内容包裹进 =main= 函数中
+ :includes :: (C/C++) 设置在代码块前 =#include= 哪些头文件
+ :import :: (D) 预先插入哪些 =import= 语句
+ :defines :: (C/C++) 在代码块前 =#define= 哪些定义
+ :libs :: (C/C++) 用于指明连接哪些库
**** Session
目前不支持Session
**** 例子
注意到,对于C语言来说,无需 =#include "stdio"=,因为这些库太常用了,总是会被include进来. 
#+begin_src C
  printf ("Hello World!\n");
#+end_src
...
当使用表格作为input数据时,表格数据会作为二元数组来看待.
#+name: somedata
| zero  |   0 |  0.23 |
| one   |   1 |  1.31 |
| two   |   4 |  4.61 |
...
默认情况下,Common Lisp解释器,永远处于session中,因此在执行代码块之前,一定要先运行SLIME session(=M-x slime=)

Common Lisp的header argument支持一个名为 ~:package~ 的header argument,用于指明是在哪个包环境下运行代码. 默认为 =:any= 表示当前package

*** 增加新语言
请按照 =org-contrib/babel/ob-template.el= 中的模板编写对新语言的支持.
** header arguments
*** 设置header arguments
...
+ no :: 默认情况,表示不插入额外的注释
+ link :: 插入额外的注释,以便从源代码中能够链接回原org文件
+ yes :: 与 =link= 一样,只是为了向后兼容
+ org :: 将一部分org文件中的内容作为注释插入源文件中. 只有代码块上面到最近的标题之间的内容会作为注释
+ both :: link + org
